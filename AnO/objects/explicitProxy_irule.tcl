when RULE_INIT {
    ...
    set static::explicitProxy_debug 0; # if this is set to 1 additional conditional logging will be enabled.
}
...
when HTTP_PROXY_REQUEST {
    ...
    #If client inserted a customer header, proxyUUID, use the value of header for upcoming logic, if not use IP address from v2 header
    if { $custHttpHeader != ""} {
    ...
    # 1. Check if this is a noh use case
    # 1. Check if sourceClient exists
    # 2. Check if customHeader exists, if yes set the srcDatagroup
    # 3. If no, then check if the sourceClient exists in the shared cidr datagroup (srcipToDatagroup)
    # 4. If no, then check if the hostname exists in Shared/explicitPxy_bypass
    # 5. If no, then check if the hostname exists in the Shared/shared_datagroup
    if {[info exists sourceClient]} {
        ...
        # Moved up above, "if" can be deleted, Leonardo comment
        if { ([HTTP::uri] contains "jndi:ldap") || ([HTTP::uri] contains "jndi%3Aldap") } {
            set 403response 1
            set hsl_proxywhitelistHost "whitelistHost=\"DENIED due to LDAP request\""
            HSL::send $hslproxy "Apache Log4j vulnerability prevention fix."
            ...
            if {[catch $srcDatagroup]} {
                if { $hostPort == 443 } {
                    ...
                    if {[class match "noh" equals Shared/$srcDatagroup ]} {
                        ...
                        log local0.debug "source datagroup match was noh use case, allowing all hosts: $srcDatagroup"
                    }
                    elseif {[class match [getfield [string tolower [HTTP::host]] ":" 1] equals Shared/$srcDatagroup ]} {
                        set hsl_proxywhitelistHost "whitelistHost=\"ACCEPTED\""
                    }
                    else {
                        ...
                        if {[class match $starHostName equals Shared/$srcDatagroup ]} {
                            set hsl_proxywhitelistHost "whitelistHost=\"ACCEPTED\""
                        }
                        else {
                            set 403response 1
                ...
                if { $static::explicitProxy_debug } {log local0.info "checking in explicitPxy_bypass datagroup"}
                if {[class match [getfield [string tolower [HTTP::host]] ":" 1] equals Shared/explicitPxy_bypass ]} {
                    set hsl_proxywhitelistHost "whitelistHost=\"ACCEPTED\""
                    set hsl_proxywhitelistHost "explicitPxy_bypass=\"True\""
                }
                else {
                    if { $static::explicitProxy_debug } {log local0.info "checking in shared datagroup"}
                    set sharedTrafficDatagroup [class match -value $sharedTraffic equals Shared/sharedDatagroup]
                    if {[catch $sharedTrafficDatagroup]} {
                        if { $hostPort == 443 } {
                            set hostName [getfield [string tolower [HTTP::host]] ":" 1]
                            if {[class match [getfield [string tolower [HTTP::host]] ":" 1] equals Shared/$sharedTrafficDatagroup ]} {
                                set hsl_proxywhitelistHost "whitelistHost=\"ACCEPTED\""
                            }
                            else {
                                set dotIndex [string first "." $hostName 0]
                                set starHostName [string replace $hostName 0 $dotIndex "#."]
                                log local0.info "REPLACED: $starHostName"
                                if {[class match $starHostName equals Shared/$sharedTrafficDatagroup ]} {
                                    set hsl_proxywhitelistHost "whitelistHost=\"ACCEPTED\""
                                }
                                else {
                                    set 403response 1
